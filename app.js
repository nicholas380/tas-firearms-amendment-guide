  ...
  <div class="modal-content">
    ...
    <section class="guide-block">
      ...
      <div class="at-a-glance-grid">
        <div><span>Who may be affected?</span><p>${(a.affected||[]).slice(0,4).map(esc).join("<br>")}</p></div>
        <div><span>Does this change existing rights or obligations?</span><p><strong>Yes</strong> — this proposal would alter the present legal or administrative position.</p></div>
        <div><span>Would lawful owners need to take action?</span><p>${a.number===2 ? "<strong>Depends.</strong> Australian citizens are generally unaffected by the citizenship requirement, while non-citizen licence holders may need to establish whether an exemption or transition arrangement applies." : "<strong>Depends.</strong> The practical action required will depend on whether the person, firearm or activity is covered."}</p></div>
      </div>
    </section>
    ...
    <section class="guide-block side-by-side">
      <div class="perspective benefit"><h3>Reasons given in support of the proposal</h3>${list(a.support)}</div>
      <div class="perspective concern"><h3>Concerns that have been raised</h3>${list(a.oppose)}</div>
    </section>
